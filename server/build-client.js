  ...
  <script>
    ...
    function render(data){
      ...
      document.getElementById("paperTrades").innerHTML =
        '<div class="tradeStats"><div><span>当前总收益</span><strong class="'+(s.profitableNow?"good":"bad")+'">'+(s.totalUnrealizedReturnPct??0).toFixed(2)+'%</strong><small>'+(s.profitableNow?"当前赚钱":"当前亏损或持平")+'</small></div><div><span>胜率</span><strong class="'+((s.winRate??0)>=50?"good":"warn")+'">'+(s.winRate??0).toFixed(2)+'%</strong><small>当前倍数 > 1x</small></div><div><span>平均当前倍数</span><strong class="'+((s.averageCurrentMultiple??1)>=1?"good":"bad")+'">'+(s.averageCurrentMultiple??1).toFixed(2)+'x</strong><small>全部记录平均</small></div><div><span>最高达到</span><strong class="good">'+(p.bestMultiple||1).toFixed(2)+'x</strong><small>'+esc(p.bestToken||"-")+'</small></div><div><span>平均最高倍数</span><strong class="good">'+(s.averageMaxMultiple??1).toFixed(2)+'x</strong><small>看卖飞空间</small></div><div><span>最大回撤</span><strong class="'+((s.maxDrawdownPct??0)>35?"bad":"warn")+'">'+(s.maxDrawdownPct??0).toFixed(2)+'%</strong><small>从最高点回撤</small></div><div><span>总记录</span><strong>'+p.total+'</strong><small>模拟自动买入</small></div><div><span>持仓中</span><strong>'+p.open+'</strong><small>持续追踪</small></div><div><span>2x+ 命中率</span><strong class="'+((s.bigWinnerRate??0)>0?"good":"warn")+'">'+(s.bigWinnerRate??0).toFixed(2)+'%</strong><small>最高倍数 ≥ 2x</small></div></div>' +
        '<div class="tradeStats"><div><span>BNB 组合</span><strong class="'+((bsc.pnlPct??0)>=0?"good":"bad")+'">'+Number(bsc.totalNative??100).toFixed(4)+' BNB</strong><small>可用 '+Number(bsc.available??100).toFixed(4)+' · 仓位 '+Number(bsc.openValueNative??0).toFixed(4)+' · '+Number(bsc.pnlPct??0).toFixed(2)+'%</small></div><div><span>SOL 组合</span><strong class="'+((sol.pnlPct??0)>=0?"good":"bad")+'">'+Number(sol.totalNative??100).toFixed(4)+' SOL</strong><small>可用 '+Number(sol.available??100).toFixed(4)+' · 仓位 '+Number(sol.openValueNative??0).toFixed(4)+' · '+Number(sol.pnlPct??0).toFixed(2)+'%</small></div><div><span>策略规则</span><strong>0.1/笔</strong><small>3x 卖 50%，0.5x 清仓</small></div></div>' +
